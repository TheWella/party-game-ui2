...
export default function PartyGameApp() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-blue-200 text-black">
      {page === "landing" && (
        <div className="p-8 w-96 text-center bg-white shadow-md rounded-lg flex flex-col gap-4">
      ...
      {page === "createRoom" && (
        <div className="p-6 w-96 text-center bg-white shadow-md rounded-lg">
          ...
          <button className="bg-blue-500 text-white p-3 rounded-lg mt-4 w-full text-lg" onClick={createRoom}>Create Room</button>
        </div>
      )}
      ...
      {page === "joinRoom" && (
        <div className="p-6 w-96 text-center bg-white shadow-md rounded-lg">
          ...
          <input type="text" placeholder="Enter Room Code" value={roomCode} onChange={(e) => setRoomCode(e.target.value)} className="p-3 border rounded-lg w-full mt-4" />
          <button className="bg-green-500 text-white p-3 rounded-lg mt-4 w-full text-lg" onClick={joinRoom}>Join Room</button>
        </div>
      )}

      {page === "lobby" && <Lobby roomCode={roomCode} socket={socket} />}
    </div>
  );
}

function Lobby({ roomCode, socket }) {
